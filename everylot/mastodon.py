...
def mastodon_image(mastodon, title_image):
    mime_type = "image/png"
    media_res = mastodon.media_post(title_image.getvalue(), mime_type)

    if (media_res.get("type") != "image"):
        LOG.error("Unable to post media! %s", media_res)
        return None

    return str(media_res.get("id", None))


def mastodon_instance(config):
...
def send_toot(mastodon, message, media_id):
    if media_id:
    ...
    try:
        ...
        LOG.error("Unable to post status! %s", err)

        if err.args[3] == ATTACHMENT_ERROR:
            LOG.error("Trying without media...")
